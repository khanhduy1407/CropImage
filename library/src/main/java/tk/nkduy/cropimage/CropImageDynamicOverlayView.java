...
class CropImageDynamicOverlayView extends CropImageOverlayView {

    private static final float CLICK_AREA_CORNER_POINT = CropImageUtils.dpToPx(24);
    ...
    public CropImageDynamicOverlayView(Context context, CropImageOverlayConfig config) {
        super(context, config);
    }

    @Override
    protected void initWith(CropImageOverlayConfig config) {
    ...
    private void initCornerPoints() {
        if (cropRect.width() > 0 && cropRect.height() > 0) {
            if (CropImageUtils.isAnyNull(Arrays.asList(cornerPoints))) {
...
//                (int) cropRect.right, (int) cropRect.bottom);
    ...
    private void onPointerMove(MotionEvent ev) {
        if (isResizing()) {
            for (int i = 0; i < ev.getPointerCount(); i++) {
                ...
                if (point != null) {
                    point.processDrag(
                            CropImageUtils.boundValue(ev.getX(i), 0, getWidth()),
                            CropImageUtils.boundValue(ev.getY(i), 0, getHeight()));
                }
            }
            ...
            cropRect = CropImageUtils.moveRectBounded(
    ...
    private class CornerPoint {
        ...
        public boolean isClicked(float x, float y) {
            ...
            CropImageUtils.enlargeRectBy(CLICK_AREA_CORNER_POINT, clickableArea);
