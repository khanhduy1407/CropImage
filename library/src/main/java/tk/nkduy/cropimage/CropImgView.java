...
import tk.nkduy.cropimage.util.MatrixUtils;
import tk.nkduy.cropimage.util.MatrixAnimator;
import tk.nkduy.cropimage.util.TensionInterpolator;

@SuppressLint("ViewConstructor")
class CropImgView extends ImageView implements OnNewBoundsListener, ConfigChangeListener {
    ...
    private CropImgViewConfig config;

    public CropImgView(Context context, CropImgViewConfig config) {
        super(context);
        initWith(config);
    }

    private void initWith(CropImgViewConfig c) {
        config = c;
    ...
    private void placeImageToInitialPosition() {
        ...
        if (config.getScale() == CropImgViewConfig.SCALE_UNSPECIFIED) {
    ...
    public void notifyImagePositioned() {
        if (imagePositionedListener != null) {
            ...
            CropImageUtils.constrainRectTo(0, 0, getWidth(), getHeight(), imageRect);
            imagePositionedListener.onImagePositioned(imageRect);
        }
    }

    private float getCurrentScalePercent() {
        return CropImageUtils.boundValue(
